...
namespace Cosmonaut.Tests
{
    public class ScalingTests
    {
        ...
        [Fact]
        public void AdjustCollectionThroughputBadResponseThrowsExcepction()
        {
            // Arrange
            var newOffer = new Offer();
            var resource = MockHelpers.CreateResourceResponse<Offer>(newOffer, HttpStatusCode.BadRequest);
            _mockDocumentClient.Setup(x => x.ReplaceOfferAsync(It.IsAny<Offer>())).ReturnsAsync(resource);

            // Act
            var action = new Action(()=>
            {
                new CosmosStore<DummyWithThroughput>(
                    _mockDocumentClient.Object,
                    "databaseName",
                    new CosmosDatabaseCreator(_mockDocumentClient.Object),
                    new CosmosCollectionCreator(_mockDocumentClient.Object), true, true, true);
            ...
            // Assert
            Assert.Throws<CosmosCollectionThroughputUpdateException>(action);
        }
    }
}